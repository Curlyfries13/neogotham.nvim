...
local Groups = require("neogotham.groups")
...
function M.extras()
  local Extra = require("neogotham.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/neogotham/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/neogotham/config.lua", "\n(--@class neogotham%.Config.-\n})")
